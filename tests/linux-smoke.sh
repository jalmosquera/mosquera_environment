...
run_smoke() {
    ...
    docker run --rm --platform "$platform" --privileged --env REPOSITORY_URL="$REPOSITORY_URL" "$image" bash -ceu '
        ...
        runuser -u mosquera -- env HOME=/home/mosquera XDG_STATE_HOME=/home/mosquera/.local/state TERM=xterm-256color bash -ceu "
            ...
            ./install workstation
            command -v fish
            test "\$HOME/.config/fish/config.fish" -ef /home/mosquera/environment/config/fish/config.fish
            command -v tmux
            test "\$HOME/.tmux.conf" -ef /home/mosquera/environment/config/tmux/tmux.conf
            command -v nvim
            test "\$HOME/.config/nvim" -ef /home/mosquera/environment/config/nvim
